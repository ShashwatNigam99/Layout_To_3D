...
from reconstruction import reconstruction, reconGT
...
# kp1, des1, vertices_original, imagePoints_original = reconGT('blendSample/3Dpoints1.npy',DATA_PATH)

vertices_original = np.array(vertices_original)
# vertices_original = CV_2_BLENDER[:3,:3] @ np.array(vertices_original).T + CV_2_BLENDER[:3,-1].reshape((3,1))#CHANGED TO CV FRAME
...
for i in range(2,FRAMES+1):
    ...
    # print(vertices_original_GT.shape, Translation.shape, tvec.shape)
    # good_new_vertices = rvec_matrix @ good_new_vertices.T  + tvec
    # vertices_original_GT =  Rotation @ vertices_original_GT.squeeze().T + Translation.reshape((3,1))
    ...
    vertices_original_GT = vertices_original_GT.reshape(-1,1,3)
